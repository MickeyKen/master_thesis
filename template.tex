...
D-UDはこれらのコンセプトを引き継ぐと共に，異なる位置の人に同時に情報投影できる効率的な情報支援を行うため，全方向四輪移動プラットフォームにパン・チルト回転機構を搭載したプロジェクタ2台から構成されている(Fig.\ref{DUD})．さらに，周囲360°のRGB情報が取得可能なカメラと距離30m，270°の範囲をデータ出力するレーザスキャナ(LRF)を搭載し，D-UDの活動環境を広範囲に認識することで人の見やすい場所に情報投影を行う人間中心の視覚的情報支援が可能である．
...
\begin{figure}[tbp]
	...
	\begin{center}
	\subfigure[Approaching human]{\includegraphics[width=6cm]{figs/dud_1.eps}
	\label{fig:lefttop}}
	%\hspace{10mm}
...
D-UDはメインPCであるNUCと，youBotの内部搭載されたPCと通信しながら，１つのシステムとして動作する．この通信は，車輪の姿勢やオドメトリなどのデータの送受信を頻繁に行うため，LANケーブルを用いて有線接続する必要がある．また，D-UDの操作時は，D-UDのNUCに無線接続したラップトップPCなどから操作することが望ましい．そこで，D-UDのNUCにトラベルルータをUSB接続し，
...
\subsection{D-UDの制御パッケージ}

\begin{figure}[tbp]
	\begin{center}
	\subfigure[Simulation of Dual Ubiquitous Display main body in GAZEBO]{\includegraphics[width=5.5cm]{figs/dud_gazebo.eps}
	\label{fig:left}}
	\hspace{15mm}
	\subfigure[URDF model of Dual Ubiquitous Display in Rviz]{\includegraphics[width=5cm]{figs/dud_urdf.eps}
	\label{fig:right}}
	\end{center}
...
\scalebox{0.9}[0.9]{
  \begin{tabular}{|c|p{10cm}|}\hline
   ノード名 & 機能説明 \\\hline
   joy\_node & kinect v2からRGBD情報を取得するノード． \\ \hline
   joy\_teleop\_node & kinect v2からRGBD情報を取得するノード． \\ \hline
   keyboard\_teleop\_node & kinect v2からRGBD情報を取得するノード． \\ \hline
   youbot\_driver & kinect v2からRGBD情報を取得するノード． \\ \hline
   move\_base & kinect v2からRGBD情報を取得するノード． \\ \hline
   amcl & kinect v2からRGBD情報を取得するノード． \\ \hline
   map\_server & kinect v2からRGBD情報を取得するノード． \\ \hline
   joint\_states\_publisher & kinect v2からRGBD情報を取得するノード． \\ \hline
   robot\_states\_publisher & kinect v2からRGBD情報を取得するノード． \\ \hline
   dynamixel\_workbench\_controllers & kinect v2からRGBD情報を取得するノード． \\ \hline
   front\_lidar & kinect v2からRGBD情報を取得するノード． \\ \hline
   rear\_lidar & kinect v2からRGBD情報を取得するノード． \\ \hline
   merge\_lidar\_node & kinect v2からRGBD情報を取得するノード． \\ \hline
   left\_projection\_node & kinect v2からRGBD情報を取得するノード． \\ \hline
   right\_projector\_node & kinect v2からRGBD情報を取得するノード． \\ \hline
   rviz & kinect v2からRGBD情報を取得するノード． \\ \hline
...
\begin{figure}[tbp]
	\begin{center}
	\subfigure[左の図]{\includegraphics[width=5cm]{sample01.png}
	\label{fig:left}}
	\hspace{20mm}
	\subfigure[右の図]{\includegraphics[width=5cm]{sample02.eps}
	\label{fig:right}}
	\end{center}
	\caption{隣り合った2つの図}
	\label{fig:twofig}
...
% ページの上(t) > ページの下(b) > 次のページ(p) の順で図の配置位置の優先度が高い　（文中に挿入(h) は扱わない）　この説明が意味不明な人は 「tex htbp」で検索
\begin{figure}[tbp]
	\centering %図を中央に配置
...
\clearpage % 改ページ：出力してない図や表を出力して改ページ   改ページコマンドには　\pagebreak とかもある
...
\begin{table}
	...
	\centering %表を中央に配置
	\begin{tabular}[tbp]{|c|c|} % |...縦枠　c...中央寄せ
		\hline % hline...横枠
		項目 & 内容 \\\hline
		あいうえお & いうえ \\
		かきく & けこかきくか \\\hline
...
\begin{table}
	...
	\centering %表を中央に配置
	\begin{tabular}[tbp]{l r} % l...左寄せ r...右寄せ
		項目 & 内容 \\
		あいうえお & いうえ \\
		かきく & けこかきくか \\
	\end{tabular}
